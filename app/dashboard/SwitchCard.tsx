...
import type { ContactSummary } from './SwitchesList'
...
export default function SwitchCard({
  ...
  const contactLabel =
    contactSummary.activeCount === 0
      ? 'No contacts yet'
      : contactSummary.activeCount === 1
        ? contactSummary.primary?.name || contactSummary.primary?.email || '1 contact'
        : `${contactSummary.activeCount} contacts`
      ...
      {mode === 'view' && (
        <>
          <dl className="grid grid-cols-2 sm:grid-cols-4 gap-x-6 gap-y-4 mb-5">
            <div>
              <dt className="text-[11px] text-[#b0b0a4] mb-0.5">Contacts</dt>
              <dd className="text-sm text-[#6b6b5e] truncate">
                <Link
                  href={`/dashboard/switches/${sw.id}`}
                  className="hover:text-[#1a1a17] underline-offset-2 hover:underline"
                >
                  {contactLabel}
                </Link>
              </dd>
            ...
            <div>
              ...
              <dd className="text-sm text-[#6b6b5e]">{formatGrace(sw.grace_period_minutes)}</dd>
            </div>
      ...
      {mode === 'delete-confirm' && (
        <div className="border-t border-[#f0f0ea] pt-4">
          <p className="text-sm text-[#6b6b5e] mb-4">
            Are you sure? This will permanently delete this switch, its contacts,
            and all check-in history. This cannot be undone.
          </p>
      ...
      {mode === 'edit' && (
        <form onSubmit={handleSave} className="border-t border-[#f0f0ea] pt-5">
          ...
          <div className="flex items-center gap-4">
            ...
            {/* hidden placeholder so plan is available on submit if ever needed */}
